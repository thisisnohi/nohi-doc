...
public class ExcelService extends FtpServer implements IDocService {
	...
	public DocVO exportDoc(DocVO doc) throws Exception {
		...
		try {
			...
			if (null != template.getTemplateSheetSize() && !"".equals(template.getTemplateSheetSize())) {
				templateSheetSize = Integer.valueOf(template.getTemplateSheetSize());
...
//	    			System.out.println("start: " + start);
	 ...
	 * @throws Exception
	 */
	private void exportToBlock(Object dataVO,DocBlockMeta block,HSSFSheet mHSSFSheet) throws Exception{
		HSSFRow row = null;//行
		...
		rowStyle = this.getRow(templateSheet, Integer.valueOf(block.getRow()));
...
//		Map<String, DocSheetMeta> sheets = template.getSheetMap();
	...
	private void paraseBlock(Object dataVO,DocBlockMeta bolckMeta,HSSFSheet mHSSFSheet) throws Exception{
		HSSFRow row = null;//行
	...
	public void setValue(Object obj , DocColMeta col,String temp) throws Exception{
		//给对象set值
